...
function subscribeToEvent(){
    ...
    var subscription = web3Quorum.eth.subscribe("logs", function(error, result){
        if (!error) console.log('New Offer proposal received');
        else console.log(error);
    }).on("data", function(log){ 
            //console.log(log);           
        let result = web3Quorum.eth.abi.decodeLog([
            deployedContracts.offerProposedEventJsonInterface.inputs[0],
            deployedContracts.offerProposedEventJsonInterface.inputs[1],
            deployedContracts.offerProposedEventJsonInterface.inputs[2]
        ],
            log.data,
            log.topics);        
         console.log(result);
